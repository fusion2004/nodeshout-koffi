...
async function main() {
  ...
  // Open the instance — every checked method returns ShoutErrorTypes.SUCCESS on
  // success, and shout.getError() yields the human-readable reason on failure.
  ...
  while (totalBytesRead < fileSize) {
    ...
    const readLength =
      totalBytesRead + chunkSize <= fileSize ? chunkSize : fileSize - totalBytesRead;
    ...
    // Get how much time we should wait before sending the next chunk and wait
